...
try
  {
	Graph_lib::Simple_window window({0, 0}, x_max(), y_max(), "Exercise 16");

	constexpr int box_size {60};
	constexpr int radius {25};
	constexpr int margin_x {50};
	constexpr int margin_y {50};

	Vector_ref<Shapes::Regular_hexagon> tiles;

	for (int i {0} ; i < (x_max() - margin_x - box_size) / box_size ; ++i)
	  {
		for (int j {0} ; j < (y_max() - margin_y - box_size) / box_size ; ++j)
		  {
			tiles.push_back(new Shapes::Regular_hexagon {margin_x + i * box_size, margin_x + j * box_size, radius});
		  }
	  }
